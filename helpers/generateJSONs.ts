...
await jb2aFreeDatabase("modules").then(() => {
	...
	fs.writeFile(
		"./JB2A_DnD5e/json/animations-object.json",
		JSON.stringify(freeDatabase, null, "\t"),
		(err) => err && console.error(err)
	);
	console.log("\tExporting flat database...");
	fs.writeFile(
		"./JB2A_DnD5e/json/database-paths.json",
		JSON.stringify(sequencerDBPaths, null, "\t"),
		(err) => err && console.error(err)
	);
});
...
await jb2aPatreonDatabase("modules").then(() => {
	...
	console.log("\tExporting animation object...");
	fs.writeFile(
		"./jb2a_patreon/json/animations-object.json",
		JSON.stringify(patreonDatabase, null, "\t"),
		(err) => err && console.error(err)
	);
	console.log("\tExporting flat database...");
	fs.writeFile(
		"./jb2a_patreon/json/database-paths.json",
		JSON.stringify(sequencerDBPaths, null, "\t"),
		(err) => err && console.error(err)
	);
});
